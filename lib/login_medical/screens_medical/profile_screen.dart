    ...
    try {
      ...
      if (userData != null) {
        setState(() {
          ...
          telefono = userData['telefono'] != null && userData['telefono'].toString().isNotEmpty
              ? userData['telefono']
              : 'Sin teléfono';
          email = userData['email'] ?? 'Sin email';
          direccion = userData['direccion'] ?? 'Sin dirección';
          imagenUrl = userData['imagen_url'] ?? '';
          codigoReferido = userData['codigo_referido'] ?? '';
          numeroReferido = userData['numero_referido'] ?? '0';
          ...
          if (userData['fecha_nacimiento'] != null &&
            ...
            try {
              final fecha = DateTime.parse(userData['fecha_nacimiento']);
  ...
  Future<void> _verificarImagenLocal() async {
    ...
    try {
      ...
      if (userData != null && userData['imagen_url'] != null && userData['imagen_url'].toString().isNotEmpty) {
        ...
        setState(() {
          imagenUrl = imagenBD;
          print("✅ Imagen actualizada desde la base de datos: $imagenUrl");
        });
      ...
      if (imagenUrlGuardada != null && imagenUrlGuardada.isNotEmpty) {
        // Verificar si es una imagen local y si existe el archivo
        if (imagenUrlGuardada.startsWith('file://')) {
          ...
          if (await file.exists()) {
            setState(() {
              ...
              print("✅ Sincronizando imagen desde SharedPreferences en Profile: $imagenUrl");
            });
            return;
          } else {
            // El archivo no existe, intentar con imagen_local_path
            print("⚠️ Archivo no encontrado: $filePath, intentando usar imagen_local_path");
            await _intentarCargarDesdeImagenLocalPath(prefs);
            return;
          }
        } else {
          // Es una imagen de red o de otro tipo
          setState(() {
            imagenUrl = imagenUrlGuardada;
            print("✅ Sincronizando imagen desde SharedPreferences en Profile: $imagenUrl");
          });
    ...
    if (imagenLocalPath != null) {
      ...
      if (await file.exists()) {
        setState(() {
          ...
          print("✅ Usando imagen local en Profile: $imagenUrl");
        });
      }
    }
  }
  
  // Método para obtener el ImageProvider correcto según el tipo de URL
  ImageProvider _getImageProvider(String url) {
    if (url.isEmpty) {
      // Si no hay URL, usar la imagen local o la imagen por defecto
      return _imagenLocal ?? const AssetImage('assets/doctor.webp');
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      _buildQuickActionCard(
                        ...
                        subtitle: 'Obtener soporte por correo electrónico',
                  ...
                  child: ListView(
                    ...
                    children: [
                      _buildFaqItem(
                        ...
                        answer: 'Para agendar una cita, navega a la sección de Categorías, selecciona tu especialidad médica, elige una fecha y hora, y confirma tu cita.',
                      ),
                      _buildFaqItem(
                        question: '¿Cómo puedo reprogramar mi cita?',
                        answer: 'Puedes reprogramar tu cita visitando tu lista de citas, seleccionando la cita que deseas cambiar, y eligiendo la opción "Reprogramar".',
                      ),
                      _buildFaqItem(
                        question: '¿Qué métodos de pago son aceptados?',
                        answer: 'Aceptamos tarjetas de crédito/débito, PayPal, y cobertura de seguros donde sea aplicable. Puedes gestionar tus métodos de pago en la configuración de la cuenta.',
                      ),
                      _buildFaqItem(
                        question: '¿Cómo puedo actualizar mi información médica?',
                        answer: 'Puedes actualizar tu información médica visitando tu perfil, seleccionando "Editar Perfil", y actualizando las secciones relevantes de la historia médica.',
                      ),
                      _buildFaqItem(
                        question: '¿Mi información médica es segura?',
                        answer: 'Sí, usamos protocolos de seguridad estándar de industria para garantizar que tus datos médicos permanezcan privados y seguros en todo momento.',
                      ),
                ...
                Expanded(
                  child: ListView(
                    ...
                    children: [
                      _buildTermsSection(
                        title: 'Introducción',
                        content: 'Bienvenido a MedApp ("nosotros", "nuestro", o "nos"). Estos Términos y Condiciones rigen su uso de nuestra aplicación móvil (la "Aplicación") y los servicios ofrecidos a través de la Aplicación (colectivamente, los "Servicios"). Al acceder o utilizar nuestros Servicios, usted acepta estar sujeto a estos Términos y Condiciones.',
                        isDarkMode: darkModeEnabled,
                      ),
                      _buildTermsSection(
                        title: 'Aceptación de los Términos',
                        content: 'Al descargar, instalar o utilizar nuestra Aplicación, usted reconoce que ha leído, comprendido y aceptado estar sujeto a estos Términos y Condiciones. Si no acepta estos términos, por favor, no utilice nuestros Servicios.',
                        isDarkMode: darkModeEnabled,
                      ),
                      _buildTermsSection(
                        title: 'Elegibilidad',
                        content: 'Usted debe tener al menos 18 años de edad para utilizar nuestros Servicios. Al utilizar nuestros Servicios, usted declara y garantiza que tiene al menos 18 años de edad y que tiene la capacidad legal para celebrar un acuerdo vinculante.',
                        isDarkMode: darkModeEnabled,
                      ),
                      _buildTermsSection(
                        title: 'Privacidad',
                        content: 'Su privacidad es importante para nosotros. Nuestra Política de Privacidad explica cómo recopilamos, utilizamos y protegemos su información personal cuando utiliza nuestros Servicios. Al utilizar nuestros Servicios, usted acepta la recopilación, uso y divulgación de información como se describe en nuestra Política de Privacidad.',
                        isDarkMode: darkModeEnabled,
                      ),
                      _buildTermsSection(
                        title: 'Licencia',
                        content: 'Sujeto a estos Términos y Condiciones, le otorgamos una licencia limitada, no exclusiva, no transferible y revocable para descargar, instalar y utilizar la Aplicación para su uso personal y no comercial.',
                        isDarkMode: darkModeEnabled,
                      ),
                      _buildTermsSection(
                        title: 'Restricciones',
                        content: 'Usted acepta no: (a) licenciar, vender, arrendar, asignar, distribuir, transmitir, alojar, subcontratar, divulgar o explotar comercialmente la Aplicación; (b) modificar, realizar trabajos derivados, desensamblar, descifrar, realizar compilación inversa o ingeniería inversa de cualquier parte de la Aplicación; (c) acceder a la Aplicación para construir un sitio web, producto o servicio similar o competitivo; o (d) copiar, reproducir, distribuir, republicar, descargar, mostrar, publicar o transmitir cualquier parte de la Aplicación en cualquier forma o por cualquier medio.',
                        isDarkMode: darkModeEnabled,
                      ),
                      _buildTermsSection(
                        title: 'Modificaciones',
                        content: 'Reservamos el derecho de modificar estos Términos y Condiciones en cualquier momento. Nos pondremos en contacto con usted de cualquier cambio material publicando los nuevos Términos y Condiciones en la Aplicación. Su uso continuo de nuestros Servicios después de tales modificaciones constituye su aceptación de los términos modificados.',
                        isDarkMode: darkModeEnabled,
                      ),
                      _buildTermsSection(
                        title: 'Terminación',
                        content: 'Podemos terminar o suspender su acceso a nuestros Servicios inmediatamente, sin previo aviso o responsabilidad, por cualquier razón, incluyendo pero no limitado a un incumplimiento de estos Términos y Condiciones.',
                        isDarkMode: darkModeEnabled,
                      ),
                      _buildTermsSection(
                        title: 'Ley Aplicable',
                        content: 'Estos Términos y Condiciones se regirán y se interpretarán de conformidad con las leyes del territorio en el que operamos, sin considerar sus disposiciones de conflicto de leyes.',
                        isDarkMode: darkModeEnabled,
                      ),
                      _buildTermsSection(
                        title: 'Contacto',
                        content: 'Si tiene alguna pregunta sobre estos Términos y Condiciones, póngase en contacto con nosotros en support@medapp.com.',
                        isDarkMode: darkModeEnabled,
                      ),
                      const SizedBox(height: 30),
                      Padding(
    ...
    try {
      ...
      if (userData != null) {
        ...
        if (resultado != null) {
          ...
          // Guardar la ruta de la imagen local para sincronizar con otras pantallas
          await prefs.setString('imagen_local_path', imagen.path);
          await prefs.setString('imagen_url', 'file://${imagen.path}');
          ...
          setState(() {
            ...
            _imagen = imagen;
            imagenUrl = 'file://${imagen.path}';
          });
          
          print("✅ Imagen de perfil actualizada y sincronizada: file://${imagen.path}");
          ...
          child: Column(
              children: [
               ...
               const SizedBox(height: 16),
                ...
                // Espacio para separar la sección de código de referido
                    ...
                    children: [
                      ...
                      Text(
                        ...
                        style: TextStyle(
                          fontSize: 16,
                          fontWeight: FontWeight.w500,
                          color: const Color(0xFF4CAF50), // Color verde para el título
                        ),
                      ),
                      const SizedBox(height: 8),
                      Row(
                        children: [
                          Text(
                            ...
                            style: TextStyle(
                              fontSize: 20,
                              fontWeight: FontWeight.bold,
                              color: const Color(0xFFFFD700), // Color amarillo para el código
                            ),
                          ),
                          ...
                          GestureDetector(
                            onTap: () async {
                              ...
                              // Función para copiar el código al portapapeles con manejo de errores
                              try {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text('Error al copiar el código. Inténtalo de nuevo.'),
                                    backgroundColor: Colors.red,
